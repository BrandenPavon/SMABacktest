...
def backtest(tick1: str, tick2: str, win: str, params: dict):
    ...
    for row, row2 in zip(df.itertuples(), df2.itertuples()):
        if (row._1 > row.SMA*(1+buy_buff)):
            if (money > 0):
                ...
                money = 0
                trades += 1
        elif (row._1 < row.SMA*(1-sell_buff)):
            if (shares > 0):
                adjusted_price = row2._1 * (1 - slippage_rate)
    ...
    print(f'Portfolio value is ${portval:.2f}, Buy buffer: {buy_buff:.3f}, Sell buffer: {sell_buff:.3f}, CAGR: {CAGR:.2f}%, Trades: {trades}, Return: {portreturn*100:.2f}%')
