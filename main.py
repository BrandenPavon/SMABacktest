...
def backtest(tick1: str, tick2: str):
    ...
    for i in range(10):
        ...
        CAGR = ((portval / 100) ** (1/11.5) - 1)*100
        avgportval += portval
        avgcagr += CAGR
        print(f'Portfolio value is ${portval:.2f}, buffer: {buffer:.3f}, CAGR: {CAGR:.2f}%, Trades: {trades}')
        buffer += 0.005
    holdportval = holdshares * df.tail(n=1)["Adj Close"].item()
    print(f'The average portfolio value is ${avgportval/10:.2f}, CAGR: {avgcagr/10:.2f}%')
    print(f'Holding would have gave Portfolio value: ${holdportval: .2f} CAGR: {((holdportval/ 100) ** (1/11.5) - 1)*100}%')

def main():
    while True:
        backtest(input("Enter First Ticker: "), input("Enter Second Ticker: "))
